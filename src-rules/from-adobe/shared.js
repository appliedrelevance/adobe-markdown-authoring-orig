...
// get the Airtable Exl Solutions Enum List
//var apiurl = 'https://experienceleague.corp.adobe.com/api/solutions?page_size=100'
//module.exports.solutions = JSON.parse(require('child_process').execSync('curl -s ' + apiurl).toString())["data"]
...
function slugify (str) {
    // console.log('> ' + str)
    var slug = encodeURIComponent(String(str).trim().toLowerCase().replace(/\s+/g, '-'))
    // console.log('< ' + slug + '\n')
    return slug
}
...
module.exports.addWarningContext =
function addWarningContext(filename, linenumber, line, rule) {
  if (line.length <= 30) {
    // Nothing to do
  } else {
