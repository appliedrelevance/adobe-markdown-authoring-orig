...
function lintContent(
  ...
  // Strip HTML code/pre blocks
  content = content.replace(/<pre.*?>[.\s\S]*?<\/pre>/g, ' CODE CODE ')
  content = content.replace(/<code.*?>[.\s\S]*?<\/code>/g, ' CODE CODE ')
  
  // Strip inline base64 binaries
  content = content.replace(/\[\]\(data:image\/([a-zA-Z]*);base64,([^\"]*)\)/, '[](inline base64 image)')
...
function lintFile(
  ...
  function lintContentWrapper(err, content) {
    ...
    try {
      ...
      console.log('[ERROR] Unknown failure in', file)
    }
  }
